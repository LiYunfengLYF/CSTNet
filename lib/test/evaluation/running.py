...
def _show_finished_seq_num(seq: Sequence, tracker: Tracker, len_dataset, debug=False):
    if debug:
        return f'0 / 0'
    else:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8, env_num=0, len_dataset=0):
    ...
    if _results_exist():
        print(
            f'  [{seq.dataset}]\t[{_show_finished_seq_num(seq, tracker, len_dataset, debug)}]\t[{tracker.name}]\t{tracker.parameter_name}\tFPS:{-1:4d}\tSequence: {seq.name}')
        return
    ...
    print(
        f'  [{seq.dataset}]\t[{_show_finished_seq_num(seq, tracker, len_dataset, debug)}]\t[{tracker.name}]\t{tracker.parameter_name}\tFPS:{int(num_frames / exec_time) + 1:4d}\tSequence: {seq.name}')


def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8, env_num=0):
