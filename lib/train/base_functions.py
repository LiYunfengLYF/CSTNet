...
def get_optimizer_scheduler(net, cfg):
    train_cls = getattr(cfg.TRAIN, "TRAIN_CLS", False)
    use_adapter = getattr(cfg.TRAIN.PRETRAIN, 'ADAPTER', False)
    rgbt = getattr(cfg.TRAIN.PRETRAIN, 'RGBT', False)
    sot_model = getattr(cfg.TRAIN.PRETRAIN, 'SOT_MODEL', False)
    auxihead_freeze = getattr(cfg.TRAIN.PRETRAIN, 'AUXIHEAD_FREEZE', False)
    ...
    if train_cls:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "cmx" in n and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "cmx" not in n and p.requires_grad],
                "lr": cfg.TRAIN.LEARN.LR * cfg.TRAIN.OPTIMIZER.BACKBONE_MULTIPLIER,
            },
        ]
        print('Use SOT_MODEL is True')
        if is_main_process():
            print("Learnable parameters are shown below for sot pretraining setting.")
            for n, p in net.named_parameters():
                if p.requires_grad:
        ...
        if is_main_process():
            ...
            for n, p in net.named_parameters():
                if p.requires_grad:
                    print(n)
    elif auxihead_freeze:
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "auxi" in n and p.requires_grad]}
        ]
        for n, p in net.named_parameters():
            if "auxi" not in n:
                p.requires_grad = False
