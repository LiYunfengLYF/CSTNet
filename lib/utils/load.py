...
def load_sot_pretrain(net, env_num=0, cfg=None):
    pretrained_path = env(env_num=env_num).pretrained_networks
    pretrained = os.path.join(pretrained_path, cfg.TRAIN.PRETRAIN.SOT_PRETRAIN_FILE)
    if cfg.TRAIN.PRETRAIN.SOT_PRETRAIN_FILE:
        try:
            net.load_state_dict(torch.load(pretrained, map_location='cpu')['net'], strict=True)
            print('Load pretrained model from: ' + pretrained)
        except Exception as e:
            print(e)
        print('Loading Finish ....')
